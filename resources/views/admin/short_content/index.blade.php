<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center justify-between nowrap w-full">
        {{ __('Short content') }}
        <x-link href="{{ route('admin.short_content.create') }}" class="px-10"><span class="font-bold">{{ __('+ ') }}</span> Buat short content baru</x-link>
      </div>
  </x-slot>
          ...
          <div class="w-full overflow-x-auto">
              <table class="w-full whitespace-no-wrap">
                  ...
                  <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                      @foreach ($short_contents as $short_content)
                                  ...
                                  <div class="flex items-center text-sm">
                                      ...
                                      <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                                          <img class="object-cover w-full h-full rounded-full"
                                              src="{{ asset("storage/uploads/short_contents/thumbnails/$short_content->image") }}" alt=""
                                              loading="lazy" />
                                          <div class="absolute inset-0 rounded-full shadow-inner" aria-hidden="true">
                                          </div>
                                      </div>
                                      <div>
                                          <p class="font-semibold truncate">{{ $short_content->title }}</p>
                                      </div>
                                  </div>
                              </td>
                              <td class="px-4 py-3 text-sm truncate">
                                  {{ strip_tags(substr(str_replace("&nbsp;", "", $short_content->body), 0, 70), null) }} ...
                              </td>
                              <td class="px-4 py-3 text-sm flex nowrap">
                                  <a href="{{ route('admin.short_content.edit', $short_content) }}"
                                  ...
                                  <button onclick="confirmDelete('{{ route('admin.short_content.destroy', $short_content) }}')"
