<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between nowrap w-full">
            {{ __('Daftar Pengingat') }}
            <x-link href="{{ route('admin.pregnancy_alerts.list.create') }}" class="px-10"><span
                    class="font-bold">{{ __('+ ') }}</span> Buat pengingat baru</x-link>
        </div>
    </x-slot>
                    ...
                    @foreach ($pregnancy_alerts as $pregnancy)
                        <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <div class="relative w-8 h-8 mr-3 rounded-full md:block">
                                        @if ($pregnancy->image)
                                            <img class="object-cover w-full h-full rounded-full"
                                                ...
                                                src="{{ asset("storage/uploads/pregnancy_alerts/thumbnails/$pregnancy->image") }}"
                                                alt="" loading="lazy" />
                                        @else
                                            <img class="object-cover w-full h-full rounded-full"
                            ...
                            <td class="px-4 py-3 text-sm flex nowrap">
                                ...
                                <button
                                    onclick="confirmDelete('{{ route('admin.pregnancy_alerts.list.destroy', $pregnancy) }}')"
